...
use agent_core::{
    ChangeExecutionPort, ExecutionPortError, FirewallExecutionPlan, IptablesRuntimeRollback,
    RollbackOutcome, RollbackReload, RollbackTarget, confirm_rollback,
    ...
    rollback_outcome, runtime_rollback_canonical_state, verify_firewall_plan_result,
};
...
use platform_linux::firewall_iptables_execute::GenericIptablesTransaction;
...
pub(crate) struct GenericIptablesExecutionPort {
    transaction: Option<GenericIptablesTransaction>,
    confirmation_runner: FirewallCommandRunner,
    execution: FirewallExecutionPlan,
    canonical_state: Option<Vec<u8>>,
    store: Arc<Store>,
...
pub(crate) struct GenericIptablesExecutionPortConfig {
    pub(crate) runner: FirewallCommandRunner,
    pub(crate) store: Arc<Store>,
    pub(crate) runtime_root: PathBuf,
    pub(crate) config_path: PathBuf,
...
impl GenericIptablesExecutionPort {
    pub(crate) fn apply(
        config: GenericIptablesExecutionPortConfig,
    ) -> Result<Self, ExecutionPortError> {
        let transaction = GenericIptablesTransaction::system(
            config.runner.clone(),
            &config.boot_id,
            config.canonical_state.clone(),
            config.execution.clone(),
    ...
    pub(crate) fn confirmation(config: GenericIptablesExecutionPortConfig) -> Self {
        Self::from_config(config, None)
    }
    ...
    fn transaction(&mut self) -> Result<&mut GenericIptablesTransaction, ExecutionPortError> {
        self.transaction.as_mut().ok_or(ExecutionPortError)
    }
...
impl ChangeExecutionPort for GenericIptablesExecutionPort {
    fn reinspect(&mut self) -> Result<(), ExecutionPortError> {
        if self.transaction.is_some() {
            self.transaction()?
                .reinspect()
            ...
            let snapshot = inspect_generic_iptables_inventory(
                &self.confirmation_runner,
                self.canonical_state.as_deref(),
                &self.boot_id,
            )
    ...
    fn arm_rollback(&mut self, _deadline_monotonic_ms: u64) -> Result<(), ExecutionPortError> {
        let (ipv4, ipv6) = self
            ...
            .rollback_artifacts()
            .map_err(|_| ExecutionPortError)?;
        create_iptables_runtime_rollback_bundle(
            &self.rollback_root(),
            &self.change_set_id,
            IptablesRuntimeRollback {
                restore_from_owned: ipv4.restore_from_owned.as_deref().map(str::as_bytes),
                restore_from_absent: ipv4.restore_from_absent.as_deref().map(str::as_bytes),
            },
            IptablesRuntimeRollback {
                restore_from_owned: ipv6.restore_from_owned.as_deref().map(str::as_bytes),
                restore_from_absent: ipv6.restore_from_absent.as_deref().map(str::as_bytes),
            },
            self.canonical_state.as_deref(),
            self.rollback_timeout_secs,
            self.max_rollback_bytes,
